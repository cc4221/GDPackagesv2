...
var _package_event_bus: Script = preload("res://addons/gdpackages/classes/package_event_bus.gd")

var _package_event_bus_cache: Script = _package_event_bus
...
func unsubscribe(event_name: String, callback: Callable) -> void:
	...
	for i in range(_subscriptions.size()):
		var sub: Dictionary = _subscriptions[i]
...
func unsubscribe_all() -> void:
	var bus: Script = _package_event_bus_cache
