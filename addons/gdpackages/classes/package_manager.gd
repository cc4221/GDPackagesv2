...
const PackageThreadedResourceManagerGlobal: Script = preload("res://addons/gdpackages/classes/package_threaded_resource_manager.gd")


static var packages: Dictionary[String, Package] = {}
static var lazy_packages: Dictionary[String, Dictionary] = {}
static var package_groups: Dictionary[String, PackedStringArray] = {}
static var group_bit_registry: Dictionary[String, int] = {}
static var _next_group_bit_index: int = 0
static var package_masks: Dictionary[String, int] = {}
...
static var _file_mod_times: Dictionary[String, int] = {}
static var _file_watcher: Timer = null

static var _packages_cache: Dictionary[String, Package] = packages
static var _lazy_packages_cache: Dictionary[String, Dictionary] = lazy_packages
static var _package_groups_cache: Dictionary[String, PackedStringArray] = package_groups
static var _group_bit_registry_cache: Dictionary[String, int] = group_bit_registry
static var _package_masks_cache: Dictionary[String, int] = package_masks
static var _adapters_cache: Dictionary[String, PackageAdapter] = _adapters
...
static func _add_package_to_tree(package_node: Node) -> void:
	...
	var root: Node = Engine.get_main_loop().get_root()
	if root:
		var node_name: String = str(package_node.name) if package_node.name != "" else str(package_node.get_instance_id())
...
static func get_all_package_names() -> PackedStringArray:
	...
	for lazy_name: String in lazy_packages.keys():
		all_names.append(lazy_name)
	return all_names

static func _get_package_config(directory: String) -> Dictionary:
	# 1. Сначала пытаемся найти и загрузить .tres конфиг
	var resource_path: String = directory.path_join("package_config.tres")
	
	# ResourceLoader.exists — единственный надежный способ проверить наличие ресурса в билде
	if ResourceLoader.exists(resource_path):
		...
		if config_resource:
			if config_resource is PackageConfig:
				# Если это наш специальный класс
				return config_resource.to_dict()
			else:
				# Если это просто ресурс, пробуем вытащить данные через свойства или мета-данные
				var dict_result: Dictionary = {}
				var fields: Array[String] = ["name", "script", "version", "description", "adapter", "dependencies", "sub_adapters"]
				for field: String in fields:
					if field in config_resource:
						dict_result[field] = config_resource.get(field)
					elif config_resource.has_meta(field):
						dict_result[field] = config_resource.get_meta(field)
				
				if dict_result.has("name") and dict_result.has("script"):
					return dict_result

	# 2. Если .tres не найден, пытаемся найти package.json
	# ВАЖНО: .json файлы будут работать в билде только если в настройках экспорта
	# в поле "Filters to export non-resource files" указано *.json
	...
	if FileAccess.file_exists(json_path):
		...
		if file:
			...
			if error == OK:
				...
				push_warning("Failed to parse JSON at %s: %s" % [json_path, json.get_error_message()])

	# 3. Если ничего не нашли
	push_warning("No package configuration found in: %s" % directory)
	return {}

static func _get_package_root(directory: String, config: Dictionary) -> Package:
	var script_value: String = config.get("script", "")
	var path: String = ""

	if script_value.begins_with("uid://"):
		var resource: Resource = ResourceLoader.load(script_value)
	...
	if loaded_script == null or not loaded_script.can_instantiate():
		push_error("Failed to load package script: " + path + " (compilation failed or missing GDExtension dependencies)")
		return null
	
	var result: Package = loaded_script.new()
...
static func get_groups_with_package(package_name: String) -> PackedStringArray:
	...
	for group: String in _package_groups_cache.keys():
...
static func add_package_to_group(package_name: String, group: String) -> void:
	...
	if not _group_bit_registry_cache.has(group):
		var bit: int = 1 << _next_group_bit_index
		_group_bit_registry_cache[group] = bit
		_next_group_bit_index += 1
	var bitval: int = _group_bit_registry_cache[group]
	var curmask: int = _package_masks_cache.get(package_name, 0)
...
static func register_packages_in_directory(directory_path: String, group: String = directory_path) -> void:
	...
	var dirs: PackedStringArray = DirAccess.get_directories_at(directory_path)
...
static func load_package(directory: String, group: String = "", dependency_chain: Array[String] = []) -> void:
	...
	if package_name in dependency_chain:
		var circular_chain: Array[String] = dependency_chain.duplicate()
		var package_index: int = circular_chain.find(package_name)
		if package_index != -1:
			var circular_part: Array[String] = circular_chain.slice(package_index, -1)
			circular_part.append(package_name)
			push_error("Circular dependency detected: " + " -> ".join(circular_part))
			
			var unique_packages_in_cycle: Array[String] = []
			for pkg: String in circular_part:
	...
	var core_value: String = config.get("core", "")
	if core_value != "":
		var core_path: String = directory.path_join(core_value)
		var core_script: Script = load(core_path)
		if core_script and core_script.can_instantiate():
			root.core = core_script.new()
		elif core_script:
			push_warning("Failed to instantiate core script: " + core_path + " (compilation failed or missing GDExtension dependencies)")
	
	var adapter_value: String = config.get("adapter", "")
	if adapter_value and adapter_value != "":
		var adapter_path: String = ""

		if adapter_value is String:
			if adapter_value.begins_with("uid://"):
				var resource_path: String = adapter_value
		...
		if adapter_path != "":
			...
			if adapter_script and adapter_script.can_instantiate():
				...
				push_warning("Failed to instantiate adapter script: " + adapter_path + " (compilation failed or missing GDExtension dependencies)")
	
	# Загрузка SubAdapters (если они есть)
	var sub_adapters_config: Array = config.get("sub_adapters", [])
	if not sub_adapters_config.is_empty():
		for i in range(sub_adapters_config.size()):
			var sub_path_value: String = sub_adapters_config[i]
			if sub_path_value is String and not sub_path_value.is_empty():
				var full_sub_path: String = _resolve_script_path(directory, sub_path_value)
				if not full_sub_path.is_empty() and ResourceLoader.exists(full_sub_path):
					var sub_script: Script = load(full_sub_path)
					if sub_script:
						var sub_instance: PackageAdapter = sub_script.new()
	...
	if root.has_signal("ready_complete"):
		root.ready_complete.connect(func() -> void: pass, CONNECT_ONE_SHOT)
...
static func load_packages_in_directory(directory_path: String, group: String = directory_path, dependency_chain: Array[String] = []) -> void:
	...
	for dir: String in dirs:
		var package_dir: String = directory_path.path_join(dir)
		print("Registering package from: %s" % package_dir)
		register_package(package_dir, group)
	
	var all_registered_packages: Array[String] = lazy_packages.keys()
	for package_name: String in all_registered_packages:
...
static func load_lazy_package(package_name: String, dependency_chain: Array[String] = []) -> bool:
	...
	var package_info: Dictionary = lazy_packages[package_name]
	var directory: String = package_info.get("directory", "")
	var config: Dictionary = package_info.get("config", {})
	var group: String = package_info.get("group", "")
	...
	if auto_load_dependencies:
		...
		for dep: String in dependencies:
			if not has_package(dep):
				if lazy_packages.has(dep):
					var new_dependency_chain: Array[String] = dependency_chain.duplicate()
					new_dependency_chain.append(package_name)
					if not load_lazy_package(dep, new_dependency_chain):
						return false
				else:
					push_error("Error, package '" + package_name + "' requires dependency '" + dep + "' which is not registered or loaded.")
					return false
	...
	if adapter_value and adapter_value != "":
		...
		if adapter_path != "":
			var adapter_script: Script = load(adapter_path)
			if adapter_script and adapter_script.can_instantiate():
				var adapter_instance: PackageAdapter = adapter_script.new(package_name)
				root.adapter = adapter_instance
				_adapters_cache[package_name] = adapter_instance
	...
	if not sub_adapters_config.is_empty():
		for i: int in range(sub_adapters_config.size()):
			var sub_path_value: String = sub_adapters_config[i]
			if sub_path_value is String and not sub_path_value.is_empty():
				var full_sub_path: String = _resolve_script_path(directory, sub_path_value)
				if not full_sub_path.is_empty() and ResourceLoader.exists(full_sub_path):
					...
					if sub_script:
						...
						var key: String = full_sub_path.get_file().get_basename()
...
static func load_all_lazy_packages() -> Dictionary:
	var results: Dictionary = {}
	for package_name: String in lazy_packages.keys():
		results[package_name] = load_lazy_package(package_name)
	return results
...
static func unload_package(package_name: String) -> bool:
	...
	var dependents: PackedStringArray = get_packages_dependent_on(package_name)
	...
	var package: Package = packages[package_name]
	
	for group: String in get_groups_with_package(package_name):
	...
	if _adapters_cache.has(package_name):
		var adapter: PackageAdapter = _adapters_cache[package_name]
...
static func unload_packages_in_group(group: String) -> Array[String]:
	...
	for package_name: String in names:
		var dependents: PackedStringArray = get_packages_dependent_on(package_name)
		var external_dependents: Array[String] = []
...
static func unload_all_packages() -> void:
	var results: Array[String] = unload_all_packages_safe()
	if results.is_empty():
		if ClassDB.class_exists("PackageEventBus"):
			PackageEventBus.emit("force_unloading_all_packages", {"package_count": packages.size()}, "PackageManager")
		
		for package: Package in _packages_cache.values():
			package._unloaded()
		_packages_cache.clear()
		
		_lazy_packages_cache.clear()
		
		for group_name: String in _package_groups_cache.keys():
...
static func emit_message_to_group(identity: StringName, message: String, group: String) -> void:
	...
	for package_name: String in packages_in_group:
		var package: Package = _packages_cache.get(package_name, null)
		if package != null:
			package._message(str(identity), message)
...
static func emit_message_to_group_mask(identity: StringName, message: String, mask: int) -> void:
	for package_name: String in _packages_cache.keys():
		var pmask: int = _package_masks_cache.get(package_name, 0)
		if pmask & mask != 0:
...
static func emit_group_message_from_package(package_name: String, identity: String, message: String) -> void:
	var groups: PackedStringArray = get_groups_with_package(package_name)
...
static func load_package_async(package_path: String, group: String = "") -> void:
	var async_loader: PackageAsyncLoader = get_async_loader()
...
static func reload_package(package_name: String) -> bool:
	...
	var package: Package = packages[package_name]
	var package_directory: String = ""
	
	if package._config.has("script"):
		var script_path: String = package._config.get("script", "")
	...
	if packages.has(package_name):
		var reloaded_package: Package = packages[package_name]
		if reloaded_package._config.has("script"):
			var script_path: String = reloaded_package._config.get("script", "")
			if FileAccess.file_exists(script_path):
				var current_mod_time: int = FileAccess.get_modified_time(script_path)
...
static func _get_package_group(package_name: String) -> String:
	for group_name: String in package_groups.keys():
...
static func _check_file_changes() -> void:
	...
	for package_name: String in packages.keys():
		var package: Package = packages[package_name]
		if package._config.has("script"):
			var script_path: String = package._config.get("script", "")
			if FileAccess.file_exists(script_path):
				var current_mod_time: int = FileAccess.get_modified_time(script_path)
				var previous_mod_time: int = _file_mod_times.get(script_path, 0)
	...
	if has_package(package_name):
		...
		if package._config.has("script"):
			...
			if FileAccess.file_exists(script_path):
				...
				var adapter_path: String = package._config.get("adapter", "")
				if not adapter_path.is_empty() and FileAccess.file_exists(adapter_path):
					_add_file_to_watch(adapter_path)

				var watch_files: Array[String] = package._config.get("watch_files", [])
				for watch_file: String in watch_files:
...
static func _reload_package_dependencies(package_name: String) -> void:
	if has_package(package_name):
		var package: Package = packages[package_name]
		var dependencies: Array[String] = package._config.get("dependencies", [])
		for dep: String in dependencies:
			if has_package(dep):
				var dep_package: Package = packages[dep]
				var dep_dependencies: Array[String] = dep_package._config.get("dependencies", [])
...
static func get_packages_dependent_on(package_name: String) -> PackedStringArray:
	...
	for pkg_name: String in packages.keys():
		var pkg: Package = packages[pkg_name]
		if pkg._config.has("dependencies"):
			var dependencies: Array[String] = pkg._config.get("dependencies", [])
			for dep: String in dependencies:
				if dep == package_name:
...
static func are_dependencies_loaded(package_name: String) -> bool:
	...
	var pkg: Package = packages[package_name]
	if not pkg._config.has("dependencies"):
		return true

	var dependencies: Array[String] = pkg._config.get("dependencies", [])
	for dep: String in dependencies:
		if not packages.has(dep):
			return false
...
static func get_missing_dependencies(package_name: String) -> Array[String]:
	...
	for dep: String in dependencies:
		if not has_package(dep) and not is_package_registered_lazy(dep):
...
static func get_package_dependencies(package_name: String) -> PackedStringArray:
	...
	var pkg: Package = packages[package_name]
	if pkg._config.has("dependencies"):
		var deps_array: Array[String] = pkg._config.get("dependencies", [])
		return PackedStringArray(deps_array)
	return PackedStringArray([])
...
static func _remove_package_from_watch(package_name: String) -> void:
	if has_package(package_name):
		...
		if package._config.has("script"):
			...
			var watch_files: Array[String] = package._config.get("watch_files", [])
			for watch_file: String in watch_files:
				if watch_file in _file_mod_times:
					_file_mod_times.erase(watch_file)

static func unload_all_packages_safe() -> Array[String]:
	var results: Array[String] = []

	# Создаем копию списка имен пакетов, которые нужно выгрузить
	var to_unload: Array[String] = packages.keys()
	var total_to_unload: int = to_unload.size()
	
	if ClassDB.class_exists("PackageEventBus"):
		PackageEventBus.emit("all_packages_unloading", {"package_count": total_to_unload}, "PackageManager")
	
	# Цикл продолжается, пока есть пакеты для удаления
	while to_unload.size() > 0:
		var unloaded_in_this_pass: bool = false
		
		# Ищем пакет, у которого НЕТ зависимых от него пакетов (никто другой его не использует)
		for i: int in range(to_unload.size() - 1, -1, -1):
			var package_name: String = to_unload[i]
			
			if can_unload_package(package_name):
				# Если пакет можно безопасно удалить (у него 0 зависимых), удаляем
				if unload_package(package_name):
					results.append(package_name)
					to_unload.remove_at(i)
					unloaded_in_this_pass = true
					# После удаления одного пакета, зависимости других могли обновиться, 
					# поэтому продолжаем цикл поиска заново
		
		# Если за целый проход по списку мы не смогли удалить ни одного пакета, 
		# значит есть циклическая зависимость или критическая ошибка
		if not unloaded_in_this_pass and to_unload.size() > 0:
			PackageLogger.log_error("PackageManager", "Cannot safely unload remaining packages due to circular dependencies: " + str(to_unload))
			break
	
	# Очистка оставшихся данных
	lazy_packages.clear()
	for group_name: String in package_groups.keys():
...
static func get_reverse_dependency_graph() -> Dictionary:
	...
	for pkg_name: String in packages.keys():
		...
		if pkg._config.has("dependencies"):
			var dependencies: Array[String] = pkg._config.get("dependencies", [])
			for dep: String in dependencies:
				if not reverse_deps.has(dep):
					reverse_deps[dep] = []
				reverse_deps[dep].append(pkg_name)
...
static func validate_dependency_chain(package_name: String) -> Dictionary:
	...
	var missing_deps: Array[String] = get_missing_dependencies(package_name)
	...
	var all_deps: Array[String] = _get_all_dependencies_recursive(package_name, [])
	for dep_name: String in all_deps:
...
static func load_resource_threaded(key: String, path: String, type_hint: String = "", cache_mode: int = 1) -> void:
	var manager: PackageThreadedResourceManager = PackageThreadedResourceManager.get_instance()
...
static func _get_all_dependencies_recursive(package_name: String, visited: Array[String]) -> Array[String]:
	...
	if has_package(package_name) or lazy_packages.has(package_name):
		var deps: PackedStringArray = get_package_dependencies(package_name)
		for dep: String in deps:
			var dep_name: String = str(dep)
			if not all_deps.has(dep_name):
				all_deps.append(dep_name)
			
			var sub_deps: Array[String] = _get_all_dependencies_recursive(dep_name, visited.duplicate())
			for sub_dep: String in sub_deps:
...
static func _resolve_script_path(directory: String, path_value: String) -> String:
	if path_value is String:
		if path_value.begins_with("uid://"):
			var resource: Resource = ResourceLoader.load(path_value)
			if resource and resource.resource_path:
